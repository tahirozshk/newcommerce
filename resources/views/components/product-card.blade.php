...
<a href="{{ route('product.detail', ['id' => $product['id'] ?? 1]) }}" class="group bg-white rounded-lg shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden border border-gray-100 block">
    <!-- Product Image -->
    ...
    <div class="p-4">
        ...
        <!-- Add to Cart Button -->
        <button onclick="event.preventDefault(); event.stopPropagation(); window.location.href='{{ route('cart') }}'" class="w-full py-2 bg-gradient-to-r from-[#FF6B35] to-[#FF8FA3] text-white font-medium rounded-lg hover:opacity-90 transition-opacity flex items-center justify-center gap-2">
